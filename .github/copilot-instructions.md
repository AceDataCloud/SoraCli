# Copilot Instructions for SoraCli

## Sync from Docs

When working on a sync task (issue labeled `auto-sync`), follow these rules:

1. **Source of truth** — Clone/checkout `AceDataCloud/Docs` and read `openapi/sora.json` (the OpenAPI spec).
2. **Compare parameters** — CLI command parameters in `sora_cli/commands/` should match the corresponding OpenAPI endpoint's request body schema.
...
4. **Update client** — If API endpoints change, update `sora_cli/core/client.py` accordingly.
